...
def distribution_nutriments_nutriscore(df_final):
    df_melted = df_final.melt(
        id_vars=['Nutriscore', 'Nom'],
        value_vars=nutriments_clés,
        var_name='Nutriment',
        value_name='Valeur'
        )
    ...
    fig = px.box(
        ...
        y="Valeur",
        color="Nutriscore",
        facet_col="Nutriment",       # Crée un petit graphique par nutriment
        facet_col_wrap=4,            # 4 graphiques par ligne
        color_discrete_map=color_map,
        category_orders=order_config,
        title="Distribution des Nutriments par Nutriscore",
        height=800                   # Assez grand pour être lisible
    )
